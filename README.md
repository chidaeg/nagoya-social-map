...
名古屋市内と隣接市町の障がい福祉サービス事業所を、カテゴリ別に検索・地図で確認できる非公式ガイドマップです。
...
- 隣接市町: 名古屋市に接する16市町村（瀬戸市・春日井市・清須市・北名古屋市・豊山町・尾張旭市・長久手市・日進市・東郷町・豊明市・大府市・東海市・あま市・大治町・蟹江町・飛島村）はWAM NETオープンデータから直接取り込み。
  ※対象者（身体/知的/精神/難病/障害児）・給食/入浴/送迎はウェルネット独自項目のため、隣接市町分では空欄。移動支援・地域活動支援など市町村独自事業も名古屋市分のみ。

現在 **名古屋市内 約6,900事業所 ＋ 隣接市町 約1,800事業所（計 約8,700件）** を収録しています（全件 緯度経度付き）。
...
- スマホ対応（一覧の開閉）
- URLパラメータによる初期絞り込み（ディープリンク）: `?q=検索語` / `?ward=区・市町名` / `?cat=カテゴリ名`（カンマ区切りで複数可）/
  `?id=事業所ID または WAM事業所番号（10桁数字）`（該当事業所へパン＆ズームして吹き出しを表示）。
  相談支援アプリ等から「この事業所／このサービスを地図で見る」リンクで開く用途。
  `cat` がどのカテゴリ名にも一致しない場合は検索語として扱う。`id` が見つからない場合は他の絞り込みだけが残る
